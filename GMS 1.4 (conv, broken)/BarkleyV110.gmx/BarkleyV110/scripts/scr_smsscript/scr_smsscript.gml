...
function scr_smsscript() {
	...
	for (i = 0; i < var_mmsize; i += 1)
	  { draw_text(var_mmx1+19,var_mmy1+4+(17*i),string_hash_to_newline(mmi[i])); }
	...
	if var_smdraw > -1 && var_smfsize != var_smdraw
	  ...
	  for (i = 0; i < sms[var_smdraw]; i += 1)
	    {
	    var_chksize = string_width(string_hash_to_newline(smi[var_smdraw,i]));
	  ...
	  for (i = 0; i < sms[var_smdraw]; i += 1)
	    {
	    var_chksize = string_height(string_hash_to_newline(smi[var_smdraw,i]));
	    if i < sms[var_smdraw]-1
	      { var_chksize += 4; }
	    if i = sms[var_smdraw]-1
	      { var_chksize += 9; }
	...
	if var_smdraw > -1
	  ...
	  draw_set_color(c_black);
	  for (i = 0; i < sms[var_smdraw]; i += 1)
	    { draw_text(var_smx1+19,var_smy1+4+(17*i),string_hash_to_newline(smi[var_smdraw,i])); }
	  ...
	  for (i = 0; i < sms[var_smdraw]; i += 1)
	    ...
	    if mouse_x >= var_smx1+4 && mouse_x <= var_smx2-2
	      ...
	      if mouse_y >= var_smy1+3+(17*i) && mouse_y < var_smy1+20+(17*i)
	        ...
	        draw_set_color(c_white);
	        draw_text(var_smx1+19,var_smy1+4+(17*i),string_hash_to_newline(smi[var_smdraw,i]));
	        draw_text(var_mmx1+19,var_mmy1+4+(17*var_smdraw),string_hash_to_newline(mmi[var_smdraw]));
