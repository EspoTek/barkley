function sItemShow(argument0, argument1, argument2, argument3, argument4) {
	...
	draw_text(x+4,y+yp,string_hash_to_newline("Power"));
	draw_text(x+4,y+yp+yps,string_hash_to_newline("Speed"));
	draw_text(x+4,y+yp+yps+yps,string_hash_to_newline("VP"));
	draw_text(x+4+xps,y+yp,string_hash_to_newline("Guard"));
	draw_text(x+4+xps,y+yp+yps,string_hash_to_newline("Brain"));
	draw_text(x+4+xps,y+yp+yps+yps,string_hash_to_newline("BP"));
	draw_text(x+4+xp,y+yp,string_hash_to_newline(global.char_rattack[ch]));
	draw_text(x+4+xp,y+yp+yps,string_hash_to_newline(floor(global.char_rfootwork[ch])));
	draw_text(x+4+xp,y+yp+yps+yps,string_hash_to_newline(global.char_rhp[ch]));
	draw_text(x+4+xp+xps,y+yp,string_hash_to_newline(global.char_rdefense[ch]));
	draw_text(x+4+xp+xps,y+yp+yps,string_hash_to_newline(global.char_rzauberacity[ch]));
	draw_text(x+4+xp+xps,y+yp+yps+yps,string_hash_to_newline(global.char_rzp[ch]));
	...
	if (d=2) draw_text(x+3+54+6+(xps*(d div 3)),y+yp+(yps*(d mod 3)),string_hash_to_newline(">"));
	else draw_text(x+3+54+(xps*(d div 3)),y+yp+(yps*(d mod 3)),string_hash_to_newline(">"));
	if (oi[d]>ni[d]) draw_set_color(c_red);
	else if (oi[d]<ni[d]) draw_set_color(c_lime);
	else draw_set_color(c_white);
	if (d=2) draw_text(x+3+8+6+54+(xps*(d div 3)),y+yp+(yps*(d mod 3)),string_hash_to_newline(string(floor(fg[d]+ni[d]))));
	else draw_text(x+3+8+54+(xps*(d div 3)),y+yp+(yps*(d mod 3)),string_hash_to_newline(string(floor(fg[d]+ni[d]))));
	}
	}
	exit;
	}
	...
	sFont(Courier8,c_white);
	if (shop<=1) {
	if (global.tEffect!="") draw_text(x+7,y+23,string_hash_to_newline(global.tEffect));
	else draw_text(x+7,y+23,string_hash_to_newline(sItemSyn()));
	} else { 
	cd=28*(pos1-1);
	...
	draw_text(x+xx+(h*xs)+137,y+yy+(q*14),string_hash_to_newline(sFormat(itemqty[i+isc],2,"0")));
	if (sho=1) { 
	if (cr!=0) {
	//draw_sprite_ext(sStartAmt,1,x+16+xx-2+(h*xs),y+yy+1+(q*14),1,1,0,vol,1); //16
	if (shop=2) { //x+xx-2+(h*xs)
	if (global.vending=0) draw_text(x+xx+(h*xs),y+yy+(q*14),string_hash_to_newline("$"+sFormat(global.tValue,4,"0")));
	else draw_text(x+xx+(h*xs),y+yy+(q*14),string_hash_to_newline("$"+sFormat(global.tValue*2,4,"0")));
	} else draw_text(x+xx+(h*xs),y+yy+(q*14),string_hash_to_newline("$"+sFormat(floor(global.tValue/2),4,"0")));
	} //x+16+xx+(h*xs)
	...
	if (dodraw=0) {
	sFont(Courier8,c_white);
	if (sibstage=-1) draw_text(x+7,y+23,string_hash_to_newline("Buy " + item[i+isc] + " " + itemdsc[b+isc]));
	}
	...
	sFont(Courier8,c_white);
	if (va!=5) draw_text(x+7,y+23,string_hash_to_newline(desc2));
	else draw_text(x+7,y+23,string_hash_to_newline("Buy " + item[i+isc] + " " + itemdsc[b+isc]));
	} else if (pos0=i) {
	sItemShow("compare",global.party[sibstage],etm,item[i+isc]);
	sFont(Courier8,c_white);
	draw_text(x+7,y+23,string_hash_to_newline("Equip " + item[i+isc]));
	}
	}
	...
	if (sibstage!=-1) draw_text(x+16+xx+(h*xs)-16,y+yy+(q*14),string_hash_to_newline(item[i+isc])); //16
	else draw_text(x+16+xx+(37*sho)+(h*xs)-16,y+yy+(q*14),string_hash_to_newline(item[i+isc])); //sell 45 is deal
	g+=1;
	}
	}
