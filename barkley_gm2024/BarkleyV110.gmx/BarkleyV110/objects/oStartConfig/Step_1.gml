...
if (instance_exists(oVoiceTest)) { if (oVoiceTest.open=1) exit; } //port: voice tester owns input while open
if (img=0) {

if (sKey(global.key_up,1) || sKey(vk_up,1)) { sP();
if (global.control=5) { global.control=9; }
else if (global.control=6) { global.control=5; }
else if (global.control=7) { global.control=6; } //port: VOICES line
...
else if (global.control=6) { global.control=7; } //port: VOICES line
else if (global.control=7) { global.control=7; }
...
else if (global.control=7) { global.sat[14]=1-global.sat[14]; } //port: Realistic Voice Acting toggle
...
else if (global.control=7) global.sat[14]=1-global.sat[14]; //port: VOICES toggles in place
